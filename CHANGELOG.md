...
- `New` 테스트 일괄실행 설정의 복제가 가능하도록 개선되었습니다.
- `New` 테스트 케이스 일람 영역에서 각종의 개선작업이 이루어졌습니다. (검색기능 추가, 정렬방식 기능 개선)
- `New` 조직 설정 페이지에서 연간계약의 Standard 요금제 신청・갱신이 가능하도록 개선되었습니다.
- `Fix` SAML Single Sign On을 사용하고 있는 사용자가 MagicPod에서 로그아웃하는 경우에도 Identity Provider에서는 로그아웃되지 않도록 개선되었습니다.


### 모바일 앱 테스트

- `New` SauceLabs에서 이용하는 Appium 버전을 1.22.1으로 업데이트하였습니다.
   - Android12에서 「키보드 입력」명령어가 실패하는 문제를 수정하였습니다.
- `Fix` 테스트 실행의 각종 안전성이 강화되었습니다.


### 브라우저 테스트

- `New` 테스트 일괄실행 설정에서 외부 클라우드 서비스 (BrowserStack, SauceLabs)의 Time Zone 설정이 가능하도록 개선되었습니다.
- `Fix` 일부 화면에서 UI 분석에 실패하는 문제를 수정하였습니다.
...
- `New` Mac 전용「Magic Pod Desktop.app」의 파일명을「MagicPodDesktop.app」으로 변경하였습니다.
   기존의 「Magic Pod Desktop.app」는 PC에서 삭제해 주세요.
   명령 행에서 기존의「Magic Pod Desktop.app」의 업데이트 명령을 실행하여도 새로운「MagicPodDesktop.app」에는 자동적으로 갱신되지 않습니다. 다운로드 사이트에서 새로운  
  「MagicPodDesktop.app」를 한번 수동으로 다운로드 하면, 그 이후로 재차 자동갱신 됩니다.      


### 전반

- `New` 테스트 일괄실행 설정에서 「테스트 결과의 표시언어」의 지정이 가능합니다.
- `New` 테스트 편집화면의 명령어 선택 팝업 관련하여 개선되었습니다. (검색기능 추가, 「모두」탭 추가)
- `New` Slack 테스트 결과 통지에 실패한 테스트명이 표시되도록 변경되었습니다.
- `New` 최대대기시잔(초), 최대횟수 등 각종 명령어의 수치 체계 파라미터를 공유 단계의 파라미터에서 가능하도록 변경되었습니다.
- `Fix` 「화면전체 드로잉이 끝날때까지 대기」명령어가 올바르게 조작되지 않는 문제를 수정하였습니다.
- `Fix` 일부 케이스에서 테스트 일괄실행 시작에 실패하는 문제를 수정하였습니다.


### 브라우저 테스트

- `Fix` 요소검출 모드에서 요소가 제대로 검출되지 않는 문제를 수정하였습니다.
- `Fix` UI 분석 로직과 요소 축적의 순서를 보다 올바르게 판정하도록 개선되었습니다.
- `Fix` 클라이언트 증명서의 업로드에 실패하는 문제를 수정하였습니다.
...
- `Fix` 「선택 상태를 보존」명령어가 올바르게 동작하지 않는 문제를 수정하였습니다.


### 전반

- `New` 테스트 일괄 실행의 실행 설정으로 Slack 통지 설정이 가능하도록 변경되었습니다.
   - 「로컬 PC」환경에서의 테스트에는 대응하지 않습니다.
- `New` 공유 Step의 정렬 방식과 검색이 공유 Step 일람 화면에서 가능하도록 변경되었습니다.
- `New` 계정 설정으로 MagicPod 표시 언어를 변경할 수 있도록 변경되었습니다.
- `New` 조직의 요금제 정보 페이지의 각종 개선이 진행되었습니다. (무료 계정 이용 기간 중에도 유료 요금제 계약이 가능한 것은 물론 무료 계정의 유효기간 표시, 연간 ・ Enterprise 요금제 계약 현황의 표시가 가능하도록 변경되었습니다.)
- `New` magic_pod_config.json의 많은 항목에서 ${...}의 기법으로 환경 변수의 참조가 가능하도록 변경되었습니다.
- `Fix` 요소 검출 모드로 취득한 요소 분석이 계속해서도 끊나지 않는 문제를 수정하였습니다.
- `Fix` 테스트 실행이 계속해서도 끊나지 않는 문제를 수정하였습니다.

### 모바일 앱 테스트

- `New` 로컬 PC 테스트에서 Xcode13.3, iOS15.4의 이용이 가능하도록 변경되었습니다.
- `New` 이용 중인 Appium의 버전을 1.22.3으로 업데이트하였습니다.
- `Fix` iOS 클라우드 단말기에서 「테스트 대상 앱을 기동」의 옵션에 「앱 정보 지우기」를 지정할 경우에 「xcodebuild failure」에러가 표출되는 문제를 개선하였습니다.


### 브라우저 테스트

- `Fix` SortableJS나 Vue.Draggable로 만들어진 요소에 대해「드래그 앤 드롭」명령어가 동작하지 않는 결함을 수정하였습니다.
- `Fix` User의 고정 IP 설정이 클라우드 브라우저에서 유효하지 않는 문제를 수정하였습니다. (Enterprise만 해당)(0.98.0이후의 버전에서 발생한 결함)
...
- `New` 무료 사용 기간을 3주간으로 단축하였습니다.
- `Fix` 각종 파일 관련 명령어의 파일 사이즈의 상한선 제한이 올바르게 작동하지 않던 문제를 수정하였습니다.
- `Fix` 각종 화면의 세세한 결함들을 수정하였습니다.

### 모바일 앱 테스트

- `Fix` Android 에뮬레이터에 위치정보가 반영되지 않는 문제를 수정하였습니다.
- `Fix` 프로젝트 복제에 실패하는 문제를 수정하였습니다.

### 브라우저 테스트

- `New` 요소 겹침 계산을 큰 폭으로 개선하였습니다.
- `New` 클라우드 브라우저에서 이용하는 Chrome의 버전을 99로 변경하였습니다.
- `Fix` 「브라우저 열기」명령어의 파라미터에 「프로세스 재기동」을 지정하면 요소 검출 모드가 제대로 작동하지 않는 결함을 수정하였습니다.
- `Fix` 요소 검출 모드에서 영역이 올바르게 검출되지 않는 문제를 수정하였습니다.
- `Fix` UI 분석에 의해 올바르지 않은 locator가 생성되는 문제를 수정하였습니다.
- `Fix` 클라우드 브라우저에서 테스트 대상 페이지 전체를 표시할 수 없는 문제를 수정하였습니다.
- `Fix` React로 작성된 페이지에서 「텍스트 입력」명령어가 올바르게 작동하지 않는 문제를 수정하였습니다.
- `Fix` Internet Explorer를 이용한 로컬 PC 테스트 실행에 실패하는 결함을 수정하였습니다. (0.97.0 이후 버전의 결함) 
